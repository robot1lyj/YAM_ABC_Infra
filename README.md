...
[文档导航](docs/README.md) · [当前验收与待部署范围](docs/acceptance.md) · [操作手册](docs/hil_quickstart.md)
...
手柄分段录制与三路MPP短集完整性均有现场记录，Thor普通/RTC和HIL也有后续联调记录。
这不是当前检查点效果或全流程通过的承诺；长时采集、完整恢复链路及本轮架构修复仍需验收/部署。开发可先从模拟模式开始。
...
HIL 通过键盘`i`或页面介入锁定，Follower保持、Leader辅助对齐，右①复用纯遥操作的绝对关节映射（无相对偏移或对齐门禁）；人工①再次锁定，页面明确交还模型。推理保留同步完整50步、TDA、训练时RTC三种路径，默认30Hz直接SDK；不再使用旧的反馈相对速度包络。切换作废旧请求和动作缓冲，录制保存实际模式与提交目标。三相机、condapi合同及现场证据见[验收](docs/acceptance.md)和[接口](docs/condapi_interface.md#推理周期与产物)。
...
| 开始或从暂停恢复 | `s` | — |
| HIL 冻结并介入 | 单击 `i` | — |
| HIL 人工阶段锁定 / 交还模型 | 界面明确交还 | ①仅锁定，不自动交还 |
...
推理/规划故障先保持，可独立重载对应子进程；录制故障可后台恢复到新数据会话，保留故障原件，不重连机械臂。恢复成功仍需明确开始；SDK/CAN故障不能由这些入口解除。新版需单独部署，不能据源码认定IPC已更新。职责与恢复合同见 [架构](docs/dagger_architecture.md)。
...
使用 `--web-port` 打开工作台时尚不构造设备；点击“连接机械臂”后就可能施加力矩、校准夹爪，**不是点击“开始”才上电**。部署必须先运行 `scripts/apply_i2rt_safety_patches.sh`；缺少线性夹爪多圈编码器安全补丁时程序会在打开CAN前拒绝连接。不带界面的CLI仍在启动时连接。退出会结束 SDK 控制并可能撤掉力矩，先支撑机械臂；软件保持不替代硬件急停。不默认写编码器零位、不运行 GELLO 校准；本站默认Follower回官方六关节零位，Leader有独立回零入口；其他配置可用已示教准备位。均需操作者明确发起并确认完整路径。
...
| 已实现、需按版本现场验收 | 训练时RTC前缀合同及局部故障独立恢复 |
| 尚未完成 | 相机曝光时钟标定；独立SDK执行层及系统盘运行时的现场迁移 |
