...
正常约每10个30Hz动作重新请求，并按缓冲剩余时间、观测到动作可用p95和安全余量动态提前；控制循环不等网络。推理/自动运动按本站2.5rad/s目标变化包络执行，人工遥操作不受该包络限制。三相机本地打包短测、condapi协议及真实Thor测量见[验收](docs/acceptance.md)和[接口](docs/condapi_interface.md#推理周期与产物)。
