...
请求除了正常新鲜度间隔，也会按缓冲剩余可执行时间、有效回复往返p95和安全余量动态提前触发；控制循环不等网络。三相机本地打包短测、condapi普通协议对接及真实Thor合同待核项见[验收](docs/acceptance.md#2026-09-15非rtc异步推理缓冲仅离线模拟)和[接口](docs/condapi_interface.md#推理周期与产物)。
...
D405 不支持三机外部硬件同步。本版采用主机接收时间配对、关节历史插值和质量指标；小偏差告警，持续过期才保持。预览与录制编码已有独立进程；控制拥有者与采集桥接尚未彻底分进程，也没有引入ROS2。
