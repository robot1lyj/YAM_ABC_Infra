...
打开 [本机工作站界面](http://127.0.0.1:8766)，点击“连接设备”，连接成功后选择模式并点击“开始”。模拟模式不打开真实机械臂、相机或 Thor。
...
- **采集工作台**：四模式卡片、三路相机、单集时长/帧数、录制/弃集、结果标记、健康与控制权。
...
使用 `--web-port` 打开工作台时尚不构造设备；点击“连接设备”后就可能施加力矩、校准夹爪，**不是点击“开始”才上电**。不带界面的CLI仍在启动时连接。退出会结束 SDK 控制并可能撤掉力矩，先支撑机械臂；软件保持不替代硬件急停。不默认清零、不运行 GELLO 校准；“回准备位”使用本机示教保存的四臂姿态，需操作员明确发起。
...
采集示范：`uv run --no-sync yam-workstation --mock --mode collect --web-port 8766`。先点击连接设备，再按 `s`，然后按 `r` 录制；结束前 `g` / `f` 标记，按 `r` 收尾，摆好物体后开始下一段。手柄②放弃当前集但不暂停遥操作；空格才暂停运动。详见 [数据采集手册](docs/collect.md)。
