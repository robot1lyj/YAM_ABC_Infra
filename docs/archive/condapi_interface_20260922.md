...
源版本：`0bc1be5`。以下包含不同阶段的旧约定，不是当前操作指引；当前内容见 [condapi_interface](../condapi_interface.md)。仅修正迁移后的相对链接，历史结论和测量不重新认证。

# condapi 对接约束（2026-09-14 架构基线 v1）

## Trained RTC 分阶段对接（2026-09-17）

已按 condapi `docs/reference/thor/13_trained_rtc_inference.md` 接入
...
H50/14D、30Hz及服务端声明的最大 `d`，回复要求 `server_timing.rtc_used=true` 且前缀不变。
不使用模型名称、后端或指纹作为3588运行记录；普通 8000 协议不变。

控制侧现由`RtcTimeline`持有30Hz目标时间轴：以三相机配对帧的**主机到达时间**
映射最近的policy tick；请求前缀取实际已提交目标历史及按同一SDK硬限位处理的未来目标，
并锁定未来前缀直到对应tick执行。回复仅从`N+d`接管，迟到直接丢弃；暂停、接管、急停、
切模式都清空时间轴并拒绝旧回复。RTC经独立Thor子进程传输，不走TDA融合、夹爪阈值trick
或100Hz二阶轨迹。页面模式切换仅重载通信子进程；**首次部署控制循环新版仍需一次受控设备
重启**。主机到达时间并非相机真实曝光时间，目前tick映射仍是近似、待真机测量。
RTC
`committed_actions` 必须是随后真正执行的目标，不能把会被可选滤波或SDK硬限位改写的旧预测
冒充已承诺动作；控制端逐tick对照SDK实际提交，若有偏差立即HOLD。2026-09-17 Thor 已在原固定 `ws://192.168.250.1:8000` 提供训练式 RTC；
...
需要 `d=33`，超过训练/握手上限 10。因此**当次服务**没有合法的 RTC 接管参数；
不得把 `d=10` 的迟到结果强行执行、放慢30Hz动作周期或静默退化成普通推理。
相机曝光到 policy tick 的物理对应与真实闭环仍未验收；详见[验收记录](acceptance_20260922.md#2026-09-17-rtc跨ipc无电机探针)。

随后 Thor 优化服务的同地址新一轮 IPC 无电机探针，已录制真实三图/14D输入、
`d=0/1/10`各5次均通过。去掉首请求后跨机往返p50/p95为217.46/219.09ms，Thor
报告推理p50/p95约198.82/199.22ms；当时36次在线页面状态采样的最旧相机帧龄
p50/p95为33.3/40.0ms。按本轮用户指定**不另加余量**，
`ceil((219.09+40.0)ms×30Hz)=8`，故 `d=8` 是离线预设，覆盖约266.7ms，
只剩约7.6ms名义空间。两个p95来自独立采样，相加只是估计，不是已测的
观测曝光→动作可用联合p95。**这是当时的离线预设，非真机验收**；
随后已实现tick归属、实际提交前缀和迟到丢弃并通过模拟，见[新验收](acceptance_20260922.md#2026-09-17-rtc控制接入与新检查点复测)。

新检查点同地址再以已录制真三图/14D和8步已提交目标连续请求13次：首请求222.73ms，
后12次往返p50/p95/max为217.84/219.29/219.35ms；均有限H50/14D且前缀逐值不变。
沿用先前相机到达帧龄p95约40ms、无额外余量，`ceil((219.29+40)ms×30Hz)=8`，
初始设`d=8`。约7.4ms只是名义空间；控制tick离散化和实际迟到率须在新版IPC上
继续观察，必要时再调至9（训练上限10），不能把独立p95相加视作已测全链路p95。

首次现场 RTC 短测揭示控制端校验顺序错误：Thor 有限右夹爪目标最高约1.025，
符合“输出可越界、控制侧限幅”合同，却被控制端先交给严格SDK目标校验，报
`invalid policy response`。现改为先对夹爪裁到[0,1]，再执行SDK关节限位/校验。
同次首请求在预定接管后2 tick才到，第二次刚好在接管tick到；`d=8`未留足
控制tick离散化空间，默认调至9，页面HOLD时可设1–10。9只改善到达余量，
不保证所有请求按时，也不代表这次RTC真机运动已验收。
...
2026-09-15 Thor侧提供上述动作合同和输出逆变换核对结论；2026-09-16 IPC已收到真实Thor一次有限50×14返回，但使用合成黑图/零状态，不能由数值本身独立证明rad/绝对目标语义或真机回放。夹爪输出变换不保证数值裁剪，本次最小值约-0.000089，因此控制侧在验证50×14有限值后裁到[0,1]，并保留原来的关节SDK硬限位与非有限值拒绝。协议测量范围见[验收](acceptance_20260922.md#2026-09-16ipc部署与真实thor无电机协议探针)。
