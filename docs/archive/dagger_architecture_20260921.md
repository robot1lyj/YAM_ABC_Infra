# 历史架构记录（截至 2026-09-21，非当前规范）

本文件保留原设计、变更与现场记录，内容可能互相覆盖；不得据此执行运动或推断部署状态。当前规范见 [四模式工作站架构](../dagger_architecture.md)。
...
部署验收：8796171已于2026-09-21经现场确认支撑后重启yam-device及Web，服务active、四臂/相机disconnected、无错误，不自动重连或运动。相关66项测试通过；完整回归427通过、3跳过，1项初始化时序失败随后专项复测通过。介入自动对齐/回零/交还高增益已部署，尚未真机动作验收；下文未部署字样为历史记录。

介入辅助对齐：TAKEOVER保留Follower姿态，Leader经三次ease-in/out靠近，峰值0.8 rad/s、2 rad/s²，夹爪不随动。按用户修正，右①无对齐精度/稳定等待门槛：以按键当刻q-h建立相对偏移，转HUMAN时清空辅助轨迹，避免首个Follower关节目标跳变。0.05 rad仅用于辅助运动完成诊断；超时停止辅助运动、锁住实测Leader但保留TAKEOVER，仍可按右①。显式暂停/急停不允许按钮恢复。交还仍显式点击，保留RTC合同。相关36项离线测试通过，现场效果待验证。

后续用户变更：删除交还模型的主从偏差门禁，RESUME与POLICY均允许消费有效动作；RTC请求也不依赖该偏差，不再因Leader偏差使在途承诺失效。承诺目标一致性、时效、急停和接管清空仍保留。RESUME Leader使用POLICY_GAIN=1.0原生位置增益；不是提升SDK上限。以下0.2 rad等待逻辑为故障历史，不是新版本交还规则。介入主动对齐的当前实现见上段；部署完成不代表现场动作已验收。

后续实测（运行37373da）：新建任务6e6af229-9b23-486d-a9c4-a652bb35264b，session_20260921_152912_7e6934/episode_000001共3260行。第一次交还在行1410，行1419进入POLICY；第二次行2414交还后直到末行保持RESUME，右Leader J4主从差从0.18044 rad增至0.94377 rad，第7个后续tick已达0.2022 rad。该区间唯一返回被丢弃，没有再次触发承诺动作异常。代码RESUME使用leader_gain=0.2，POLICY才用1.0；就绪丢失解释了未执行，低增益与偏差扩大吻合，但未做增益因果真机对照。本轮不擅改交还逻辑/阈值。

用户要求Leader回零改用Follower同类原生位置控制增益：仅leader_homing路径使用gain_scale=1.0（原0.4），原生Kd不变，普通HOLD及人工重力补偿不变。相关36项离线测试通过，尚未部署/真机验收，不为此重启设备。
