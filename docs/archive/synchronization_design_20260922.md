...
源版本：`0bc1be5`。以下包含不同阶段的旧约定，不是当前操作指引；当前内容见 [synchronization_design](../synchronization_design.md)。仅修正迁移后的相对链接，历史结论和测量不重新认证。
...
`policy_fusion=raw`不融合；普通模型默认`tda_smooth`按openarm-vr队列算法丢弃请求期间已消费的新块前缀（最多25步），再对旧队列尾部与新块头部的完整重叠段做线性混合，含夹爪。历史固定短窗口`smooth`与多块`ensemble`均已移除。没有有效动作或请求超时就保持，不延长末动作；RTC前缀条件另走独立路径，不经TDA。推理/自动关节的反馈相对目标包络为3rad/s，不是真实速度保证。2026-09-17的普通10w＋TDA对照将`policy_trajectory_hz=0`，即30Hz目标直接给YAM SDK；100Hz二阶代码保留但本轮关闭，失败的纯线性100Hz插值不恢复。
