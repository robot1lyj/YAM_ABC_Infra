...
**当前源码差距**：`hil/web.py` 与 `dataset_workbench/web.py` 均将uvicorn绑定127.0.0.1，TrustedHostMiddleware也只允许本机；现有8766/8767不是已开放的局域网产品端口。本轮是架构/记忆写回，尚未改服务代码、系统防火墙或完成跨机实测。后续agent须同时处理监听、Host/客户端地址、部署参数与用户路径；旧SSH转发说明只属于临时开发方式。
