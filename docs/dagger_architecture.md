...
保留的明确依赖：推理及正式录制仍要求有效任务、相机观测；相机连接仍做编码器预热，避免长录制冷启动积压，该路径尚未拆成单独的编码器就绪状态；未创建策略客户端的旧会话不能凭修改地址直接补建客户端。后两项不宣称已完全解耦。录制异常仍可主动请求HOLD，SDK反馈超时与CAN故障仍走设备故障恢复，不随UI需求放宽。
