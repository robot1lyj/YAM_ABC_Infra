...
默认非RTC异步重规划：最多一个Thor请求在途，正常约每10个30Hz动作取最新观测预取，缓冲截止时间不足时按“观测到动作可用”p95动态提前；控制线程只非阻塞轮询回复。动作索引0绑定本机观测参考时刻，返回后按`action_dt`物理移除过期前缀；新块替换未来计划而非追加旧队尾。可选`raw`、短窗口`smooth`、`ensemble`；新旧观测网格相位不同时，对旧关节预测在新块精确目标时刻插值，只融合相同物理目标时刻且不外推。夹爪始终采用最新块并裁到[0,1]。epoch变化、急停、接管、保持与故障清缓冲，旧结果由token拒绝；请求超时/缓冲耗尽沿用保持策略。
