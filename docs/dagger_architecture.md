...
2026-09-15新增可选的按角色CPU亲和性：目标IPC服务预留4–5号A76给控制拥有者与四路读数、6–7号给MPP/HDF5编码进程，页面/相机/预览/录制桥接使用0–3号A55。开发机无环境配置时不绑核。它先解决已观察到的全部工作站子进程共占4–7号核的问题；控制拥有者仍是工作站Python进程内线程，GIL与内存/USB/磁盘带宽仍共享。仅当部署后录制对照、队列斜率和跟随延迟证明有收益时保留该分区；独立控制进程和NVMe临时缓冲是后续需要实测支持的升级，不由此宣称已完成。
