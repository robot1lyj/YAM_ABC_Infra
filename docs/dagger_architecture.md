...
1. **四臂经用户的 hub 接 IPC，先登记身份。** 两 follower、两官方电动 leader；第一步P0见 [设备身份表](workstation.md#第一步设备身份登记-p0)。用户已确认USB Hub连接各机械臂USB-CAN接口；Hub型号、适配器S/N和物理拓扑待核验，板载 `bcan0`～`bcan3` 仅是已枚举资源，不再作为默认接臂方案。CAN仍需每臂独立逻辑通道；USB Hub可以共享USB上联，但不能据此把各臂并为同一CAN总线。正式角色 `can_left/can_right/can_lead_l/can_lead_r` 按已登记身份稳定绑定；不得按枚举顺序猜左右/主从。
...
用户最新顺序：**先做P0四臂与三相机序号识别，再做部署与运行联调。** 以下为任务边界，未创建或启动 agent。各 agent 先读 `AGENTS.md`、本节，再只读本包所有者；不要重构已冻结的四模式。软件修改在所属仓库提交，现场操作需要独立核验现场条件。

| 包 | 所属 / 范围 | 输入与依赖 | 完成交付与闸门 |
|---|---|---|---|
| **P0 身份登记，第一步** | YAM：识别USB Hub及四臂本体/USB-CAN适配器、三相机序列号和角色 | 已登记IPC；设备接入条件。只读枚举，必要的最小枚举工具独立说明，不先部署完整控制栈 | 交付4臂＋3相机身份表、hub拓扑、稳定标识、未识别项与原始证据；不启用电机 |
