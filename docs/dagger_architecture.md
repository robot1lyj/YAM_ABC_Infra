...
后续用户变更（本地实现，未部署）：删除交还模型的主从偏差门禁，RESUME与POLICY均允许消费有效动作；RTC请求也不依赖该偏差，不再因Leader偏差使在途承诺失效。承诺目标一致性、时效、急停和接管清空仍保留。RESUME Leader使用POLICY_GAIN=1.0原生位置增益；不是提升SDK上限。以下0.2 rad等待逻辑为故障历史，不是新版本交还规则。介入时主动对齐Leader到保持中的Follower是下一阶段方案，尚未实现，需有界轨迹、完成反馈及暂停/急停覆盖，不可等同当前锁定介入。
