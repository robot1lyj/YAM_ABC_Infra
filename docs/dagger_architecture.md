...
- 常驻`yam-device`进程内的`Runtime`是四臂上层命令的唯一写入者，并独占CAN、相机、推理与录制意图；录制数据/落盘由每设备会话独立子进程承接。可独立重启的`yam-workstation` Web/API只通过受限Unix socket转发命令。Web重连先请求HOLD，不继承运动；停止设备进程才会释放硬件。控制线程不等待Thor、录制进程、视频编码、HTTP或转换；录制故障仍以信号使其HOLD。
...
当前正式部署已把Web/API与常驻设备进程拆开；本地待部署版本再把`RecordingSession`及NVMe暂存迁到录制owner子进程，设备控制进程只保留录制意图和有界、非阻塞提交。Web进程可单独重启；设备进程仍独占CAN/相机/安全状态，其重启仍按真实设备断开处理，必须先支撑机械臂。录制owner子进程在当前版本随设备会话创建/结束，活动录制中不可无损重启。
