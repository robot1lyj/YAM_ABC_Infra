...
控制循环只提交到有界录制队列；后台编码与JSONL记录。每路编码器单线程，避免三个编码器自行扩张线程数。状态面板低频更新最近300样本的分阶段p50/p95/p99，记录队列峰值与编码耗时。会话关闭、设备关闭后才执行LeRobot整理；批量Parquet/流式图像写入，不依赖RK上的PyTorch。真实性能需现场测量，见 [验收](acceptance.md)。
