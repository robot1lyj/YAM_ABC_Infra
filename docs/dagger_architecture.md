...
- 常驻`yam-device`进程内的`Runtime`是四臂上层命令的唯一写入者，并独占CAN、相机、安全仲裁与录制意图；Thor通信/序列化、动作块规划、录制落盘分别由独立子进程承接。可独立重启的`yam-workstation` Web/API只通过受限Unix socket转发命令。Web重连先请求HOLD，不继承运动；停止设备进程才会释放硬件。控制线程不等待Thor、动作规划、录制进程、视频编码、HTTP或转换；录制故障仍以信号使其HOLD。此进程边界变更截至2026-09-17仅离线验证，IPC迁移与真机验证待完成。
...
Web/API与常驻设备进程已拆开；录制编码和Thor通信各有子进程。2026-09-17本地新增`yam-action-planner`：它在后台把有限50×14回复按同步完整块或TDA转换成带时间轴/来源的计划；`raw`算法仅留内部回归测试，不作为操作模式。设备进程只非阻塞消费已安装的计划，保留CAN/相机、HOLD/接管/急停、动作时效及最终限位。规划进程失联会使当前策略进入HOLD，旧回复经epoch拒绝；HOLD下可分别重载Thor通信或动作规划子进程，不释放力矩。这个新版**尚未部署IPC**，首次迁移仍需一次受控`yam-device`重启。之后修改现有动作块规划算法只需重载规划子进程；修改设备侧通用计划接口、安全仲裁或SDK写入仍需要设备重启，不能宣称全部控制代码热替换。录制owner子进程随设备会话创建/结束，活动录制中不可无损重启。
