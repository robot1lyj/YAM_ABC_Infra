...
- 常驻`yam-device`进程内的`Runtime`是四臂上层命令的唯一写入者，并独占CAN、相机、策略仲裁与录制意图；Thor通信/序列化、录制落盘分别由独立子进程承接。可独立重启的`yam-workstation` Web/API只通过受限Unix socket转发命令。Web重连先请求HOLD，不继承运动；停止设备进程才会释放硬件。控制线程不等待Thor、录制进程、视频编码、HTTP或转换；录制故障仍以信号使其HOLD。
...
Web/API与常驻设备进程已拆开；录制编码和Thor通信各有子进程。设备进程保留CAN/相机、动作时间对齐、接缝融合与安全仲裁；策略通信子进程可在HOLD独立重载，调整接缝模式/步数可在HOLD直接生效。修改设备进程中的动作缓冲或仲裁源码仍须重启设备进程并释放力矩，**尚未做到所有推理控制代码热替换**。录制owner子进程随设备会话创建/结束，活动录制中不可无损重启。
...
默认非RTC异步重规划：最多一个Thor请求在途，正常约每10个30Hz动作取最新观测预取，缓冲截止时间不足时按“观测到动作可用”p95动态提前；控制线程只非阻塞轮询回复。动作索引0绑定本机观测参考时刻，返回后按`action_dt`物理移除过期前缀；新块替换未来计划而非追加旧队尾。执行模式为`raw`直接换块或可关闭的短窗口`smooth`；新旧观测网格相位不同时，对旧关节预测在新块精确目标时刻插值，只在两块接缝处融合相同物理目标时刻且不外推。夹爪始终采用最新块并裁到[0,1]。历史多块`ensemble`已因现场效果不佳退役。epoch变化、急停、接管、保持与故障清缓冲，旧结果由token拒绝；请求超时/缓冲耗尽沿用保持策略。
