...
- 常驻`yam-device`进程内的`Runtime`是四臂上层命令的唯一写入者，并独占CAN、相机、推理与录制状态；可独立重启的`yam-workstation` Web/API只通过受限Unix socket转发命令。Web重连先请求HOLD，不继承运动；停止设备进程才会释放硬件。控制线程不等待Thor、视频编码、HTTP或转换。
...
当前8766用户服务仍把Web/API、Runtime和设备SDK放在同一OS进程：静态页面可热更新，但Python后端重启会执行`close_hil()`并撤掉四臂力矩。目标隔离应是常驻设备控制进程独占CAN/相机/安全状态，Web/API作为可独立重启的客户端经本机IPC提交意图；拆分前任何后端重启都按真实设备断开处理，必须先支撑机械臂。
...
2026-09-15新增可选的按角色CPU亲和性，并在RK3588 IPC试验控制4–5、编码6–7、其余0–3。真实双臂运动A/B与操作员反馈表明限制控制在两核明显削弱跟手性；正式服务回滚至已验证的4–7号A76掩码，开发机无环境变量仍不绑核。这个失败试验不证明录制落盘是控制瓶颈：运动录制的会话队列峰仅2帧，不录制也发生控制间隔尖峰。控制拥有者仍是工作站Python进程内线程，GIL与内存/USB/磁盘带宽共享。停录后的三层写入排空已从固定25/30/35秒总期限改为有保存进展就持续等待、120秒完全停滞报故障。下一阶段应以不缩窄控制核心为前提，将采集桥接/暂存和控制分进程，再现场比较运动中的p99间隔、跟随延迟、长时队列斜率及停录排空；允许停录后封装，不允许录制期间丢失有效数据。
