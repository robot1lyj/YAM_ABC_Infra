...
第一版已接入运行代码，使用 [三模式教程](hil_quickstart.md)；非RTC异步推理、官方leader接管、
线程采集/软件配对、连续记录和专家段导出已离线验证。尚未进行真机性能/任务成功率验收。
下面保留原方案与审查记录，标为“尚未接入”的历史条目以本段及教程为准。
用户允许放宽同步要求；多进程/曝光时钟校准不作为第一版前置条件。
