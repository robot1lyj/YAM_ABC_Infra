...
- 真实公开Web入口启用控制口令和单页面控制租约。口令来自YAM_CONTROL_TOKEN，未设置时首次生成data/workstation/control.token（0600）；不通过API返回、不提交Git。浏览器每页独立会话；只有当前操作者心跳送达设备，其他标签页不能掩盖其失联。3秒失联后需显式新操作领取控制权，不自动恢复运动。已认证的其他页面仍能暂停/软件急停。
- 私有Unix设备接口不重复做浏览器鉴权；安全依赖Unix socket目录权限。HTTP口令不替代TLS或隔离网络，生产网络仍须限制访问。模拟入口默认不启用口令。
