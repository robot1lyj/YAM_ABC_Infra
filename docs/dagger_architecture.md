...
默认非RTC异步重规划：最多一个Thor请求在途，执行有效缓冲时用最新观测预取新块，控制线程只非阻塞轮询回复。动作索引0绑定本机观测参考时刻，返回后按`action_dt`物理移除过期前缀，保留原始索引供记录；新块替换未来计划而非追加旧队尾。可选`raw`、短窗口`smooth`、`ensemble`，后两者只比较目标时刻差不超过`action_dt/4`的预测；ensemble最多5块、新预测权重大，夹爪始终采用最新块。epoch变化、急停、接管、保持与故障清缓冲，旧结果由token拒绝；请求超时/缓冲耗尽沿用保持策略。`--baseline`保留不预取分块用于比较，不使用RTC或修改Thor模型。
