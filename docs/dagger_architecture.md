...
Web/API与常驻设备进程已拆开；录制编码和Thor通信各有子进程。设备进程保留CAN/相机、动作块处理与安全仲裁；策略通信子进程可在HOLD独立重载，普通模型的`raw/tda_smooth`动作块模式可在HOLD切换。修改设备进程中的动作缓冲或仲裁源码仍须重启设备进程并释放力矩，**尚未做到所有推理控制代码热替换**。录制owner子进程随设备会话创建/结束，活动录制中不可无损重启。
...
普通非RTC异步重规划：最多一个Thor请求在途，正常约每10个30Hz动作取最新观测预取，缓冲不足时按延迟p95提前；控制线程非阻塞轮询。`raw`按观测时刻丢过期前缀并替换未来计划；默认`tda_smooth`复用openarm-vr队列规则，按请求期间消耗步数丢新块前缀，再混合旧队列尾部和新块开头的完整重叠段，含夹爪。RTC训练模型不经过TDA；目标tick和前缀执行合同待另验。epoch变化、急停、接管、保持与故障清缓冲，旧结果由token拒绝；请求超时/缓冲耗尽沿用保持策略。
