...
### HIL正式使用前录制边界修复（2026-09-21，本地待部署）
...
故障注入入口：`tests/test_architecture_recovery.py`（9项）；全量`pytest -q`为416通过、3跳过、17子测试通过，前端`node --check`通过，记忆检查无错误；本地8893模拟页面可打开。本轮未部署IPC、未操作机械臂；不能将离线通过写成真机验收。
