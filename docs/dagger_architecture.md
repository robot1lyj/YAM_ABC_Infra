...
2026-09-15新增可选的按角色CPU亲和性，并在RK3588 IPC试验控制4–5、编码6–7、其余0–3。真实双臂运动A/B与操作员反馈表明限制控制在两核明显削弱跟手性；正式服务回滚至已验证的4–7号A76掩码，开发机无环境变量仍不绑核。这个失败试验不证明录制落盘是控制瓶颈：运动录制的会话队列峰仅2帧，不录制也发生控制间隔尖峰。控制拥有者仍是工作站Python进程内线程，GIL与内存/USB/磁盘带宽共享。下一阶段应以不缩窄控制核心为前提，将采集桥接/暂存和控制分进程，再现场比较运动中的p99间隔、跟随延迟、长时队列斜率及停录排空；允许停录后封装，不允许录制期间丢失有效数据。
