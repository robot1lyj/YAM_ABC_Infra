...
介入对齐实现：TAKEOVER保留Follower的介入姿态，Leader目标从自身反馈经三次ease-in/out到该姿态，时间按峰值0.8 rad/s和2 rad/s²计算；夹爪输入维度不动。用原生位置增益，0.05 rad容差、轨迹完成且3tick稳定才接受右①（初始已在容差内可直接接受）；按钮时复核当前实测偏差。提前按不排队。轨迹时长+3秒超时转HOLD并锁住实测Leader，暂停/急停/交还清理对齐状态。交还明确点击后立刻请求新观测，不增加固定等待，仍遵守RTC承诺tick。下段“尚未实现”描述为此前方案历史；本版已经实现，现场动作验收仍待用户。
