...
30秒右臂运动窗口最大约42ms且操作员明确反馈顺滑。正式服务配置因此恢复
`CPUAffinity=4 5 6 7`，不设置角色亲和性环境变量；源码可选能力保留，
但未经新一轮现场运动验收不得在IPC重新启用两核控制方案。用户确认四臂支撑后已
通过页面断开四臂、四CAN DOWN，再重启用户服务；新主PID`29438`确认为
`CPUAffinity=4-7`，页面与设备均未连接。部署/安全重启状态
与数据完整性见[现场记录](workstation.md)。该配置仍不是独立控制进程或硬实时保证。
