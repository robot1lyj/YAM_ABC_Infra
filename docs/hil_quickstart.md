...
## 控制台故障恢复与访问（2026-09-21，待部署）

DAgger录制边界补充（待部署）：人工按①锁定后直接交还，仍是一集；页面“暂停运动”结束一集，随后“交还模型”先开启新集再恢复。开集请求无法进入录制队列时继续保持，并显示录制错误。介入期间切换模式必须确认“结束介入并切换”，当前集结束、四臂保持，随后需显式开始；不能用切换模式绕过介入门禁。

真实Web首次写操作会要求控制口令。可通过服务环境变量`YAM_CONTROL_TOKEN`预设；否则由Web生成`data/workstation/control.token`，仅服务器本机账号可读，不将口令放进URL或日志。每个浏览器标签页独立，只有正在操作的页面续心跳；切换操作页面先关闭原页面，等待超过3秒后在新页面明确点击操作。旁观页可查看状态，输入有效口令后始终可请求暂停/软件急停。局域网HTTP仍须放在可信隔离网络中。
