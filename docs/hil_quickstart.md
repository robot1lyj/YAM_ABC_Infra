...
“回准备位”是当前控制循环执行的四臂关节插值流程，模型/遥操作在此期间不拥有控制权。本站`factory_zero_home: true`按YAM官方定义以四臂各六关节的零角度为目标，**不写电机编码器零点**，且Follower及Leader夹爪各自保持当前开度；不需示教或保存准备位。其他站可先在重力补偿下手扶摆位，结束补偿后保存当前准备位。关节最大插值速率0.12rad/s、到位误差阈值0.015rad、最长60秒；超过时间保持并报错。没有碰撞规划，点击前须确认从当前姿态到零位的完整路径安全。
