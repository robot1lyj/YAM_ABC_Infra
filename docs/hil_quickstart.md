...
HIL新版介入：Follower保持介入时姿态，Leader自动沿三次平滑轨迹对齐，峰值不超过0.8 rad/s、2 rad/s²，使用原生位置增益；手柄夹爪不随动。误差≤0.05 rad且运动结束连续3tick稳定后显示可按右①（起始已在容差内无需额外等待）。提前按右①不会排队；按键时再次检查实际两臂误差，避免解锁跳变。轨迹结束后3秒仍不就绪则暂停；页面暂停/急停取消对齐。此阶段不录入专家数据。人工①锁住后仍需页面显式交还，不自动启动模型。
