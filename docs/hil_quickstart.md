...
4. 标记结果、结束录制；离开设备前先支撑机械臂，点击“断开并保存”。原始数据保存后即可重新连接，后台LeRobot转换会自动暂停。
...
工作台逐集提交MP4＋HDF5，断开机械臂后后台转换LeRobot；无界面CLI仍生成 `会话/lerobot/`，见 [字段和格式](convert.md)。运行时只异步录制原始数据；`--raw-only`用于暂缓格式整理和性能测量。遥操作/推理模式两个手柄按钮均无功能。
