...
HIL新版（尚未部署）交还不再受主从0.2 rad偏差门槛限制，等待模型动作期间Leader使用原生位置增益；模型动作必须通过时效和RTC合同检查。不要强推高增益保持中的Leader。旧版主从就绪提示属于历史：若Follower已在零位，可暂停并确认完整路径安全后让Leader回零；不能将回零当成任何RTC错误的通用解法。
