...
正式部署要求局域网以IPC地址和8766（可配置）直接跨平台浏览器访问；配置、数据平台8767及验证边界归 [局域网交付](dagger_architecture.md#局域网访问与跨平台交付)。**当前代码仍绑定本机**，下文127.0.0.1命令是已有开发入口，本轮未开放LAN服务；日常产品使用不把SSH隧道或本机命令行作为前置。
