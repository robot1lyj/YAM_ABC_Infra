...
| HIL交还模型 | 界面 | 锁定后明确点击；之后Leader才开始对齐 |
| 暂停运动 | 空格 | 界面暂停；不占手柄按钮 |
...
HIL介入：Follower保持，Leader辅助对齐（峰值0.8 rad/s、2 rad/s²），右①随时停止辅助对齐并复用纯遥操作的绝对关节映射，不要求误差达标或稳定等待，不建立相对偏移；夹爪复用同一软接管逻辑。若两臂未对齐，Follower会追向Leader实际位置，不能承诺首帧不跳变。辅助对齐超时停止并锁住Leader，仍可右①接管；显式暂停/急停则不允许按钮绕过。等待段不录入专家数据。人工①锁住后仍需页面显式交还，不自动启动模型。此绝对映射修改尚待部署。
