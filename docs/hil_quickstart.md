...
新版页面在模型执行时显示运行设备报告的`100 Hz 线性插值`或`100 Hz 二阶轨迹`；旧设备进程未重启前缺少该字段，不能以磁盘配置判断实际模式。
...
推理/HIL页面显示Thor往返p95、动作缓冲余量、过期裁剪步数及关节限速。HOLD且未录制时可在页面切换`raw/smooth`、设置1–12步的两块接缝窗口；提交后控制线程清空旧缓冲、递增epoch，下一次明确开始才执行新设置。改动只保存在当前设备会话，断开重连恢复station YAML默认。4步在30Hz下跨约100ms；网络延迟用于过期前缀裁剪和预取预算，**不能严格推出最佳接缝步数**，需固定任务对照摆动、到位和夹爪配合。页面不开放关节限速改写。
...
高频轨迹写入可用`policy_trajectory_hz`显式开关；`0`保持原30Hz直接路径。本站下一次获准设备部署的配置为`policy_trajectory_mode: linear`、100Hz：以一个30Hz控制周期为标称时长，从上次已提交位置线性插到当前目标，关节仍保留3rad/s限速及SDK硬限位，夹爪不经过该关节插值。它不再使用二阶临界阻尼和30rad/s²加速度限制；因属因果插值，即使目标未触发限速也会增加约一个控制周期的到位时间，若限速介入或电机写入超期还可能更久，并非精确跟踪保证。保留`second_order`作为可回滚配置，但更改该底层模式需重启设备进程并会释放力矩，不能在四臂未支撑时执行。策略是唯一Follower写线程；进入HOLD、急停、接管、遥操作、维护或重力补偿前先停止并join，再恢复原直接IO。页面控制权显示`Thor 模型 / 100 Hz 轨迹`。现有在线设备若未受控重启，仍运行旧二阶模式，页面不会因磁盘代码更新自动切换。

2026-09-16旧二阶方案的82秒raw录制离线重放：4步交接使换块后4帧内关节目标最大单帧跳变的95%值从0.373降到0.140rad；8步为0.061rad但对新raw目标的95%最大关节偏差增至0.228rad。来源为IPC `session_20260916_164308_bb674f/episode_000001`，只证明接缝目标曲线变化，不是真机效果验收，也不是新线性插值的验证结果。新模式须先通过离线单步/反向/限速/暂停测试，再在现场短测下降到位、接缝摆动及夹爪闭合时序。
