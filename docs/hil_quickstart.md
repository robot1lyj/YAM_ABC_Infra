...
| 开始或从保持恢复 | s | 开始/恢复 |
| HIL人工接管/交还策略 | i（单击） | 接管按钮；运行中任一leader顶部按钮上升沿，250ms整站去抖 |
| 保持 | 空格 | 保持按钮；leader第二按钮按住期间持续保持 |
| 采集开关录制 | r | 采集模式遥操作中按顶部按钮；保持时顶部按钮先启动遥操作 |
...
每次运行的会话目录中，`episode_000001/` 等子目录各保存一段。会话结束写入 `session.json` 索引；未开始采集不创建 episode。新episode包含 `steps.jsonl`、三路分片MP4、结束时生成的 `manifest.json`。
