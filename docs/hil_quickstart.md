...
浏览器打开 http://127.0.0.1:8766 。页面初始不连接设备。每日晨检可不选任务、不接相机，直接
连接机械臂后开始遥操作；该模式不录制。正式采集先创建/选择任务，再分别连接相机和机械臂；
...
1. 每日开机检查直接连接机械臂，系统自动进入不录制的遥操作晨检；正式采集才先新建或选择任务并连接相机。模型模式连接机械臂时填写Thor WebSocket地址。真实连接确认现场固定、清空行程、有人照看；构造设备就可能施力矩与校准夹爪。
...
两臂一起接管。HIL与普通遥操作都从主从当前位置建立相对接管偏移，不要求操作者先精确对齐，
