...
1. 普通遥操作直接连接机械臂；正式采集可先新建或选择任务并连接相机，也可从无任务遥操作先暂停保持、连接相机、首次选任务后切换采集，四臂无需断开。新版允许HOLD且结束介入/维护、录制保存完成后切换任务：旧数据保留，新建数据会话，不断臂（首次部署需更新设备服务）。模型模式连接机械臂时填写Thor WebSocket地址。真实连接确认现场固定、清空行程、有人照看；构造设备就可能施力矩与校准夹爪。
...
仅非零位站把示教准备位保存到本机 `data/workstation/ready_real.json`；mock使用独立文件，与station配置SHA256绑定，配置变化后不会加载旧准备位，不提交Git。不要在浏览器多开多个控制操作员会话；页面已有单一操作会话租约；旁观页不能抢占心跳，任何页面仍可暂停。

重力补偿时四臂关节可手动摆放，Follower夹爪保持。Follower沿用SDK重力补偿与阻尼，退出后恢复原位置控制参数；2026-09-14在腕部相机安装条件下通过现场定性检查，长时下垂与精确末端负载未量化。关节点动可在任意工作模式的HOLD、未录制、无未结束介入或维护/锁存时使用，无需切换到采集；单次有限步长；这些维护动作不进入训练片段。

## 局部故障恢复

2026-09-22代码更新需单独部署；以下不表示IPC已更新。恢复均不自动运动：
- 推理通信/非法响应：HOLD后点击「重载推理通信」；规划超时/退出点击「重载动作规划」。就绪后明确开始，维护和遥操作不依赖模型重新上线。
- 录制失败：暂停并结束介入/维护后点击「恢复录制服务」。后台封存旧会话、创建新录制进程及数据会话，机械臂与相机不重连；等待结果后重新开始。若在HIL介入中失败，可明确结束介入并切遥操作，先保持再恢复。
- 相机初始化失败：排除原因后重新连接；不会永远停在连接中。机械臂初始化失败、预检和相机就绪后可重试初始化向导。
- SDK反馈过期、CAN错误仍锁定硬件故障；不能用录制/推理恢复绕过。关闭失败也不能当作已经释放设备。
- 数据集后台进程退出：失败作业不会自动重跑；检查已生成文件后显式重新提交，新worker独立建立。
- 离线恢复使用 `python -m yam_abc_reproduce.hil.recovery SOURCE --output NEW_DIRECTORY`，合并可恢复片段和暂存缓冲，保留原件。坏缓冲逐项报告；缺少manifest不能猜帧率恢复。

耗时恢复期间其他设备变更请求立即提示忙，不会让超时按钮点击稍后突然生效；状态查看与暂停仍可用。系统盘运行时/数据盘挂载防回退配置见 [环境](environment.md)。
...
Leader回零与策略镜像使用原生位置控制增益；人工时Kp/Kd归零，保留SDK重力与已开启的库仑摩擦补偿。暂停锁定按当前交互规则保持，不通过放宽硬件故障检查改善手感。真实到位能力和手感以现场验收为准。
...
| HIL人工结束 | 手柄① | 高增益锁定原地；不自动交还 |
| HIL交还模型 | 界面 | 锁定后明确点击；新观测推理，不设主从偏差门禁 |
| 暂停运动 | 空格 | 界面暂停；不占手柄按钮 |
...
两臂一起接管。HIL人工阶段直接复用普通遥操作的 Leader→Follower 绝对1:1关节映射、人工增益和夹爪软接管，不保留相对偏移。介入时Leader辅助对齐，但右①不等待误差达标；未对齐时Follower会追向Leader目标，不能承诺首帧不跳变。
交还时取新观测推理，不设主从就绪偏差门槛；旧请求和旧块无效。
...
`THOR_IP`替换为实际值。纯推理使用 `--mode inference`。未配置URL的会话可在HOLD通过页面设置来源并独立建立通信，无需断开机械臂。网络错误暂停策略并结束当前录制；重载成功仍保持，需明确开始。
...
页面在模型执行时显示设备进程报告的策略轨迹状态；不能由浏览器缓存或磁盘文件推断运行算法。实验性纯线性100Hz轨迹已删除，历史失败证据保留在[验收](archive/acceptance_20260922.md#2026-09-16-线性插值推理短测与回退)。
...
2026-09-16旧二阶方案的82秒raw录制离线重放：4步交接使换块后4帧内关节目标最大单帧跳变的95%值从0.373降到0.140rad；8步为0.061rad但对新raw目标的95%最大关节偏差增至0.228rad。来源为IPC `session_20260916_164308_bb674f/episode_000001`，只证明接缝目标曲线变化，不是真机效果验收。纯线性插值失败短测及调度尖峰见[现场验收](archive/acceptance_20260922.md#2026-09-16-线性插值推理短测与回退)。
...
三种模式不能共用一个含糊的时间定义：同步块从回复开始按30Hz执行完整50步；TDA按请求期间队列消耗对齐；训练时RTC按请求携带的目标tick及承诺前缀对齐。动作间隔不得随控制循环频率改变。各模式合同见上节及架构文档。
...
数据字段完整定义见 [HIL原始字段表](hil_dataset_fields.md)。当前录制侧同时去掉takeover_wait与handback_wait，每集独立清单；保留帧使用连续frame_index/timestamp，原始tick/time不改。规则在每次新集加载，修改该规则不用重启机械臂。现场需核对运行版本和新集字段，本轮未复测IPC；旧第02集仅修正多余摘要，不改原视频/动作帧。

HIL介入：Follower保持，Leader辅助对齐（峰值0.8 rad/s、2 rad/s²），右①随时停止辅助对齐并复用纯遥操作的绝对关节映射，不要求误差达标或稳定等待，不建立相对偏移；夹爪复用同一软接管逻辑。若两臂未对齐，Follower会追向Leader实际位置，不能承诺首帧不跳变。辅助对齐超时停止并锁住Leader，仍可右①接管；显式暂停/急停则不允许按钮绕过。等待段不录入专家数据。人工①锁住后仍需页面显式交还，不自动启动模型。当前代码采用此绝对映射；实际部署版本须查看设备运行状态，不以历史记录推断。

常见故障以中文原因及操作建议展示，保留原始诊断：RTC动作一致性、SDK反馈超时、CAN未启动、录制队列满、模型响应无效、回放起点不符、磁盘满及网络超时。未知英文异常也显示中文引导，不猜测根因。提示不自动执行恢复、不解除安全门禁。

HIL新版（2026-09-21已部署8796171）交还不再受主从0.2 rad偏差门槛限制，等待模型动作期间Leader使用原生位置增益；模型动作必须通过时效和RTC合同检查。不要强推高增益保持中的Leader。旧版主从就绪提示属于历史：若Follower已在零位，可暂停并确认完整路径安全后让Leader回零；不能将回零当成任何RTC错误的通用解法。

心跳恢复修正：同一标签页使用sessionStorage保留控制身份，刷新不丢身份；原操作者短暂失联后恢复心跳会清除失联提示，但不会请求start或交还。Web刚重启尚无操作者时，允许可见页面通过心跳建立控制会话；已有操作者时其他页面心跳不能抢占。3秒失联暂停仍保留，修改仅涉及Web，无需重启设备服务。相关回归37项通过。

DAgger录制边界补充：人工按①锁定后直接交还，仍是一集；页面“暂停运动”结束一集，随后“交还模型”先开启新集再恢复。开集请求无法进入录制队列时继续保持，并显示录制错误。介入期间切换模式必须确认“结束介入并切换”，当前集结束、四臂保持，随后需显式开始；不能用切换模式绕过介入门禁。

按用户要求取消控制口令及登录弹窗，不再读取或生成口令文件，旧YAM_CONTROL_TOKEN设置无效；同时移除内嵌浏览器不支持的window.prompt调用。每个浏览器标签页独立，只有正在操作的页面续心跳；切换操作页面先关闭原页面，等待超过3秒后在新页面明确点击操作。旁观页可查看状态，也可请求暂停/软件急停。保留同源检查及运动安全门禁，局域网HTTP须放在可信隔离网络中。

录制/任务切换异常只限制相关操作，不应使整个页面失效。切换超时先查看当前任务及录制错误，不重复点击创建；不确定的录制会话禁止继续写入，以免串任务。普通保存可以继续等待，管理请求的3秒确认上限不是视频保存时限。

界面状态短暂超时仍可点击暂停/软件急停尝试送达；页面切换和只读查看不依赖任务操作成功。若网络或SDK真的故障，软件停止不保证生效，使用实体急停。新的实现需部署后生效，不表示运行中的旧进程已升级。
