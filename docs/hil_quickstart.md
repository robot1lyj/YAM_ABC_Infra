...
高频轨迹写入可用`policy_trajectory_hz`显式开关；`0`保持原30Hz直接路径。本站2026-09-16经现场支撑和许可受控重启后，设备进程已加载`policy_trajectory_mode: linear`、100Hz；页面连接四臂后报告`linear`且处于HOLD，**未验证新模式的真机运动效果**。线性模式以一个30Hz控制周期为标称时长，从上次已提交位置插到当前目标，关节仍保留3rad/s限速及SDK硬限位，夹爪不经过该关节插值。它不使用二阶临界阻尼和30rad/s²加速度限制；因果插值即使未触发限速也增加约一个控制周期的到位时间，限速介入或写入超期可能更久。保留`second_order`作为回滚配置；切换底层模式需重启设备进程并释放力矩，须先支撑四臂。策略是唯一Follower写线程；进入HOLD、急停、接管、遥操作、维护或重力补偿前先停止并join，再恢复原直接IO。
