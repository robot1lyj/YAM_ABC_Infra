...
浏览器打开 http://127.0.0.1:8766 。页面初始不连接设备。点击“连接设备”，连接后先保持，再点击“开始”执行；不需要真实机械臂或Thor。
...
1. 点击“连接设备”，填写任务说明，模型模式填写Thor WebSocket地址。真实连接确认现场固定、清空行程、有人照看；构造设备就可能施力矩与校准夹爪。
2. 选择四种模式之一。切换会结束旧片段并保持，不立即运动；点击开始执行。操作按钮根据实际状态启用。
3. 采集时查看三路画面、录制时长、已写帧数、当前控制权。手柄①/②规则保持不变。
4. 标记结果、结束录制；离开设备前先支撑机械臂，点击“断开并保存”。等待LeRobot整理完成后才能重新连接。

`--output` 在交互界面中作为会话根目录，每次连接创建带时间和随机后缀的新子目录；不带界面的CLI仍直接写到指定目录。浏览器心跳超过3秒未更新时，独立监测线程请求暂停并取消待执行动作；页面恢复不会自动运动。这不是对进程卡死、断电或总线失效的硬件保护。
...
控制、采集与录制沿用线程；仅预览使用共享内存与独立编码进程，不称为硬实时框架。
