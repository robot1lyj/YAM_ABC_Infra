...
会话队列默认约10秒容量（30Hz时300个采集时刻），这是本项目RAM保护参数，不是相机/MPP硬限制；每项主要包含三路RGB引用及该时刻状态。单集桥接队列32帧，编码进程仍有自身有界缓冲；满或写盘失败明确aborted。停录后允许有进展地继续排空，120秒完全无进展才报故障；当前实现仍要求持续编码速度跟上采集，不能靠无限增大RAM队列支持长时间积压。长录制的目标方案是磁盘暂存/分段后停录继续编码归档，尚未部署，不承诺断电后未完成MP4/manifest完整。
