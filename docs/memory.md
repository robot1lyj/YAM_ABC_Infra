...
| 入口 | 内容与预算 | 加载条件 |
|---|---|---|
| AGENTS.md | 必须遵守的工作/产品/设备边界；≤6 KiB | 项目工作规则，宿主已注入则不重复读取 |
| cache/kernel.md | 稳定产品原则、核心约束和owner路由；≤3 KiB，最多8条主题 | 需要项目概况时 |
| cache/context_index.md | 一级任务导航；≤4 KiB；具体问题表转 [problem_index](cache/problem_index.md) | 选择本次owner时，不跟随全部链接 |
| cache/checkpoint.md | 当前目标、最近结果、活跃阻断、下一步和来源；≤3 KiB | 续作时，不作为历史日志 |

默认路径AGENTS＋kernel＋一级index合计≤13 KiB；确需续作再加checkpoint，总计≤16 KiB。它们是文档维护预算，不是完整会话token限制，也不限制长期资料、按需技术章节或累计检索量。更换文件名/新增热摘要不能规避合计；若修改预算，在本owner说明原因，不静默增长。
