...
2026-09-16复核固定Kai0`9d93078`的[Agilex temporal smoothing](https://github.com/OpenDriveLab/kai0/blob/9d93078c757840f50e75248c5c5a94ab7b41e13a/train_deploy_alignment/inference/agilex/inference/agilex_inference_openpi_temporal_smoothing.py)与[temporal ensembling](https://github.com/OpenDriveLab/kai0/blob/9d93078c757840f50e75248c5c5a94ab7b41e13a/train_deploy_alignment/inference/agilex/inference/agilex_inference_openpi_temporal_ensembling.py)。YAM保留可关闭的两块短窗口平滑，使用旧100%→新100%线性权重；历史多块融合试验已退役。差异保留：YAM按本机观测时间而非消费计数裁剪；关节只在接缝处融合同一物理目标时刻；连续夹爪不平均、不套ARX的二值阈值，始终取最新块；缓冲耗尽时不复用last_action或填充旧尾部。没有Kai0式第0步时间偏移配置；不移植其ROS、动作单位或自动回位。原生YAM合同和ABC-DiT训练/推理均把夹爪当连续14D中的第6/13维，ABC只做通用归一化/反归一化，模拟器适配层才把[0,1]缩放到物理行程。
