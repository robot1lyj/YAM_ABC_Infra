...
`policy_fusion=raw`不融合；普通模型默认`tda_smooth`按openarm-vr队列算法丢弃请求期间已消费的新块前缀（最多25步），再对旧队列尾部与新块头部的完整重叠段做线性混合，含夹爪。历史固定短窗口`smooth`与多块`ensemble`均已移除。没有有效动作或请求超时就保持，不延长末动作；RTC前缀条件另走独立路径，不经TDA。推理/自动关节的反馈相对目标包络为3rad/s，不是真实速度保证。现有策略专用100Hz二阶轨迹仍启用；纯线性100Hz插值历史真机效果不佳，未恢复。
