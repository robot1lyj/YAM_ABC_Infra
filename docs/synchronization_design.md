...
`policy_fusion=raw`不融合；本站默认可关闭的`smooth`在新旧块重叠的前1–50步按旧100%→新100%线性过渡（单步重叠取旧关节），现场页面限调1–12步。相机观测与30Hz控制时钟不锁相，因此旧预测先在新块的**精确目标时刻**做线性插值；不外推，也不把相邻时刻误当成同一目标。夹爪始终取最新预测且裁到[0,1]，不平均开闭。历史多块`ensemble`现场效果不佳，已移除。没有有效动作或请求超时就保持，不延长末动作。本版不采用RTC前缀条件。本站当前把推理/自动关节的反馈相对目标包络设为3rad/s；它不是基于上一提交目标的真实速度/加速度保证。策略专用100Hz二阶轨迹已启用，但响应滞后和调度尖峰仍待量化；纯线性100Hz插值真机效果不佳，已移除。
