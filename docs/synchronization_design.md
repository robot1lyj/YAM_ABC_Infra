...
现场 Thor 与 RK3588 通过以太网连接：Thor 做本地模型推理，RK3588 采集和控制。
本文是升级后的目标设计；当前 hil/ 只有普通块控制核心和进程内帧缓存，
尚未实现以下共享内存、多频率执行、校准时钟与完整真机同步。不能称其已成熟落地。
采用 Diffusion Policy / UMI 的带时间戳异步实机结构，借鉴 ros2_control 的
read-update-write 单拥有者生命周期。第一阶段保留 i2rt SDK，不因同步需求强制全栈迁移 ROS2。
...
这与当前已测试的无预取基准核心不同，需作为下一调度实现并单独测试。
