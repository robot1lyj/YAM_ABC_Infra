...
控制循环只提交有界队列，序列化/编码在后台；每个视频编码器限制一个线程。`status.performance`低频统计最近300样本的IO读取、配对、仲裁、提交和总工作时间，单位毫秒；`record_metrics`记录队列峰值/编码耗时。数据格式整理在会话结束和设备关闭之后运行；正式数据目录只在转换成功后可见。开发机结果以 [验收报告](acceptance.md) 为准，不代替RK3588实测。
