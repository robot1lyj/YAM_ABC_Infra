...
参考Kai0的非RTC异步路径，当前采用独立推理与时间戳动作缓冲。每个块绑定本机观测参考时刻、request_id、epoch及模型采样间隔；最多一个请求在途，旧块有效时预取，回复后丢弃已过期前缀并替换尚未执行的未来计划，不把新块接在旧队尾。控制循环不等待Thor。
...
`policy_fusion=raw`默认不融合；可选`smooth`在新旧块相同目标时刻的前1–50步线性过渡，或`ensemble`聚合最近2–5块对相同目标时刻的关节预测。目标时刻容差为`action_dt/4`，不混合相邻时刻；夹爪始终取最新块，不平均开闭。较老预测超过动作年龄门限不再参与；没有有效动作或请求超时就保持，不延长末动作。本版不采用RTC前缀条件，步长限幅也不构成实际速度/加速度保证。参数与启动命令见[运行手册](hil_quickstart.md#非rtc推理与可选动作融合)。
