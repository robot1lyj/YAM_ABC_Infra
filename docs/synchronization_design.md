...
`policy_fusion=raw`不融合；本站默认可关闭的`smooth`在新旧块重叠的前1–50步按Kai0旧100%→新100%线性过渡（单步重叠取旧关节），或`ensemble`聚合最近2–5块对相同目标时刻的关节预测，按Kai0/ACT较早预测优先指数权重，衰减默认0.01。相机观测与30Hz控制时钟不锁相，因此旧预测先在新块的**精确目标时刻**做线性插值；不外推，也不把相邻时刻误当成同一目标。夹爪始终取最新预测且裁到[0,1]，不平均开闭。没有有效动作或请求超时就保持，不延长末动作。本版不采用RTC前缀条件。本站推理关节目标按与上游默认一致的1.5rad/s逐周期限幅，这是命令变化包络，不是实际速度/加速度保证。
