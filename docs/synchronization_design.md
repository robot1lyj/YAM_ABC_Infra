...
每tick计算最新块的有效尾部：`min(观测时刻+H50×action_dt, 观测时刻+action_timeout)`，减去当前本机monotonic时刻。剩余时间≤`max(初始总往返估计,最近16个有效回复本机RTT p95)+prefetch_margin`时，允许越过正常`replan_period`提前请求；否则按200ms新观测节流。请求在途时不提交第二个任务，不累积旧观测；答复有效才更新RTT样本，超时/epoch变化仍按原安全状态机处理。初始200ms估计包含用户约130ms模型耗时的保守外部预算，真实网络/服务端往返未测，不能作为性能结论。IPC三相机本地整理/打包短测与范围见[验收](acceptance.md#2026-09-15非rtc异步推理缓冲仅离线模拟)。

`action_dt` 必须与训练数据匹配，不能把模型的 50 步预测长度、去噪次数或控制频率混为一谈。第一版使用动作索引 0 对应观测参考时刻的约定，真实部署前要核验训练样本的观测/动作对齐；不匹配时应改契约，不靠增大超时解决。

`policy_fusion=raw`默认不融合；可选`smooth`在新旧块相同目标时刻的前1–50步按Kai0旧100%→新100%线性过渡（单步重叠取旧关节），或`ensemble`聚合最近2–5块对相同目标时刻的关节预测，按Kai0/ACT较早预测优先指数权重，衰减默认0.01。目标时刻容差为`action_dt/4`，不混合相邻时刻；夹爪始终取最新预测且裁到[0,1]，不平均开闭。较老预测超过动作年龄门限不再参与；没有有效动作或请求超时就保持，不延长末动作。本版不采用RTC前缀条件，步长限幅也不构成实际速度/加速度保证。参数与启动命令见[运行手册](hil_quickstart.md#非rtc推理与可选动作融合)。
