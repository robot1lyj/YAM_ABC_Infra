...
2026-09-15核心资源隔离第一阶段：现场旧服务PID `taskset -pc`实测允许CPU4–7，
其页面、三相机、录制桥接、预览和MPP子进程都继承同一掩码，并非真正的调度资源隔离。
IPC `lscpu -e`显示0–3号核最高1.8GHz、4–7号最高2.304GHz；临时无设备Python
进程从0–3号亲和性显式改到4–5号成功，证明新服务分区在当前系统可配置。
源码已加入按角色的可选线程/进程亲和性与服务配置：UI/相机/桥接/预览0–3，
控制及四臂读数4–5，编码进程及其FFmpeg6–7。未设置这些环境变量的开发机保持
原调度；这阶段尚未在IPC实测相机帧率、录制吞吐或四臂延迟，不能据配置声明性能改善。
控制拥有者仍与UI/相机/桥接共享Python解释器/GIL，内存带宽和NVMe也未隔离。
...
- 2026-09-14 已完成四臂1–6号电机逐个通讯/OFF及两follower官方零重力初始化；左右
  ...
  手柄按钮转换和成对遥操作仍未验收；未写零位或发送全零Home。
