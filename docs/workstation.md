...
本文的枚举、延迟和故障段落均为带日期的现场快照，不代表设备此刻在线。当前操作以[采集手册](collect.md)、[部署边界](deploy.md)为准；实测范围与日期以各节证据为准。2026-09-22已修复现场gs_usb中确认缺失的上游生命周期补丁；原故障完整因果链及NVMe掉盘根因仍未确定。

## 2026-09-22 USB-CAN / NVMe 内核核查与驱动修复

9月21日傍晚日志先出现四臂通信失败，随后Renesas xHCI错误与Hub断连；17:41的新启动
在NVMe的`fe150000.pcie`发生Link Fail，18:20的新启动恢复。没有已确认的运行中NVMe
热移除证据，也未确定两次重启方式。USB和NVMe位于不同PCI域，仍未排除板级电源因素。
USB主控制器确认为Renesas uPD720201 `1912:0014 rev03`；四CANable经Realtek RTS5411
`0bda:5411` Hub `5-2`接入，同一Renesas另挂一台D405（USB3 `6-4`）。

实际gs_usb源码确认缺失RX回收、open失败清理、TX失败释放三类修复，已回移五项上游
提交并加载`6.1.118-yam1`。四口80次静默停启通过；双Leader另40次重开、4000次官方
手柄读取全部回包，无收发错误。没有构造SDK或控制电机；不能据此替代运动与长测。
维护结束NVMe live、/data在线、四CAN DOWN，device/Web恢复、机械臂和相机断开；
相机重连被现有页面控制权拒绝，未抢占。完整时间线、证据边界及后续BSP核查见
[现场报告](evidence/20260922-ipc-usb-nvme.md)，安装和回滚归[环境说明](environment.md#2026-09-22-生命周期修复已加载)。
...
| 闭合端（同一运行坐标分支，rad） | 第一轮 | 第二轮 |
|---|---:|---:|
| 官方函数返回的瞬时最大值 | 6.55012589 | 6.53944457 |
| 末三次稳定采样中位数 | 6.37769894 | 6.37960632 |
| 左侧原保存值 | 6.90441531 | 6.90441531 |

左侧原保存闭合值比本次稳定端点更靠里约 **0.525 rad**，也超过两轮瞬时峰值。
用原配置下发 `0.15`，原始目标为约 **6.05468 rad**；以本轮稳定端点换算，
有效行程开度约 **6.3%**，而右侧更新后的同值目标约为 **15%**。这属于位置映射比较，
不是夹持力或毫米宽度的测量，但已支持“左侧原标定带额外闭合偏置，左右归一化值不等效”。
不能把右侧复测正常推成左右映射都没有问题，也不能仅凭此归因每次抓空。

本轮按用户的测试范围保留左侧原配置 **`[0.62123, -5.04368]`**，未自动改成新稳定值，
也未把左侧旧值复制给右侧；改变标定口径会改变模型动作与采集反馈，须另行确认后再验证。
...
本节为官方采样配置与核查历史；当前文件已按下节用户要求覆盖闭合端为 6.9 rad。
...
随后已从工作台新版“目标开度 0～1＋应用”分别下发 `0.15`，最终左反馈
`0.14991739099202656`、右反馈 `0.14947822957898538`，四臂 HOLD、无录制/调节错误。
用户随后现场确认该次两侧开度一致（视觉核查，不是毫米测量）。首轮复用反馈递推造成不到位，已改为命令目标递推，
保留暂停取消和超时停止；右侧一次被操作台心跳暂停打断，恢复后显式重新应用才到位。
本地/IPC各62项回归通过；使用增量补丁，不将本地其他架构变更整文件覆盖到旧IPC基线。
详见[页面0.15核查](evidence/20260922-gripper-015-ui-check.json)。

## 2026-09-22 用户指定闭合端覆盖为 6.9 rad

用户随后认为约 6.54 rad 的闭合端不够，明确要求左右都改为 **6.9 rad**。
当前配置为左 `[6.9, 1.2289234760051873]`、右 `[6.9, 1.2434195468070488]`；
张开端不变。本次采用用户指定的 6.9，而非左侧旧配置换算后的精确值 6.904415307179586。
这属于**人为闭合偏置**，不应称为官方标定或实测硬限位，原始采样证据不变。
相同归一化目标会更靠近闭合端；接近 0 的目标可能持续顶住机械端点并增加发热风险。
本次仅同步文件，不更新在线 SDK、不重连或运动；下一次连接读取，效果尚未真机验证。
先前 0.15 视觉一致的确认仅适用于官方极值配置，不自动迁移为本次覆盖值的验收。
