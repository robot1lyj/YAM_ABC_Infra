...
def test_resume_leader_offset_does_not_invalidate_rtc_promises(reply_arrived):
    ...
    rows = np.tile(action(.8), (50, 1))
    rows[:9] = commitment.actions
    if reply_arrived:
        assert a.accept_rtc(token, rows, .03, 1, limits)
    epoch = a.epoch
...
def test_hil_takeover_clears_rtc_and_resume_rejects_old_reply():
    ...
    for i in range(70):
        leader = a._leader_frozen.copy()
        a.step(q, leader, now=.05+i/30, dt=1/30, policy_tick=i+2)
